...
router.post('/', rejectUnauthenticated, async (req, res) => {
  const userId = req.user.id;
  console.log('userId inside POST achievements:', userId);
  ...
  try {
    ...
    let { rows: userTotalXpPoints } = await connection.query(userTotalXpPointsQuery, [userId])
    userTotalXpPoints = Number(userTotalXpPoints[0].xp_points)
    console.log('userTotalXpPoints:', userTotalXpPoints);
